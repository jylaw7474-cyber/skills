...
| step | how |
|---|---|
| colour roles | A colour drawn mostly as polylines is line work; a fine hatch covering tens of m² is an area zone. Override with `--zone-colors` / `--line-colors`. |
...
| area zones | Exact m² per hatch colour, summed from the drawing's triangles. |
...
| spaces | Free area pinched apart at anything narrower than a door, then grown back, so rooms do not leak into each other through their doorways. |
...
`--door`, `--min-run`, `--frames 1,4` to build a subset.
...
and an area schedule taken from the hatch geometry.
